...
async function load() {
  ...
  const data = await api<Page<QuestionBank>>(`/api/v1/question-banks/favorites?${params}`)
  pageInfo.value = data
...
<template>
  <section>
    ...
    <div class="my-4 grid max-w-3xl gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <input v-model="keyword" class="rounded-md border border-slate-300 bg-white px-3 py-2" placeholder="搜索收藏题库" @keyup.enter="search" />
