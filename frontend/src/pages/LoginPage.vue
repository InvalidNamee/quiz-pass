...
<template>
  <div class="flex min-h-[80vh] items-center justify-center">
    <form class="grid w-full max-w-sm gap-3 rounded-lg border border-slate-200 bg-white p-8" @submit.prevent="submit">
      ...
      <div class="relative">
        <input v-model="password" class="w-full rounded-input border border-slate-300 bg-white py-2.5 pl-3 pr-10" :type="showPw ? 'text' : 'password'" placeholder="密码" autocomplete="off" />
      ...
      <AppButton :loading="loading" class="w-full justify-center py-2.5" @click="submit">登录</AppButton>
      <p class="text-center text-sm text-slate-600">
        还没有账号？<RouterLink class="font-medium text-brand-600 hover:text-brand-700" to="/register">注册</RouterLink>
      </p>
    </form>
  </div>
</template>
