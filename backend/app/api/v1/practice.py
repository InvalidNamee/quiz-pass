...
def list_mistakes(bank_id: int, page: int = 1, page_size: int = 20, resolved: bool | None = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not _can_read_bank(bank, current_user):
        raise HTTPException(status_code=404, detail="Question bank not found")
    stmt = select(MistakeRecord).join(Question, MistakeRecord.question_id == Question.id).where(MistakeRecord.user_id == current_user.id, MistakeRecord.bank_id == bank_id)
    if resolved is True:
    ...
    return page_response([_mistake_out(item, questions_by_id[item.question_id]) for item in items if item.question_id in questions_by_id], total, page, page_size)
